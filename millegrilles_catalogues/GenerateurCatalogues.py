...
class Generateur:
    ...
    def generer_catalogue_applications(self):
        ...
        :return:
        """
        path_catalogues = path.join(self._args.src or self.__repertoire_src)
        path_archives_application = path.join(self._args.signed or self.__repertoire_signed)
...
def parse_commands():
    ...
    parser.add_argument(
        ...
        help="Input path des fichiers de catalogue"
    )
    parser.add_argument(
        '--signed', type=str, required=False,
        help="Output path des fichiers de catalogue signes (.json.xz)"
    )
